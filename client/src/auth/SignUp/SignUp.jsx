...
import {
  Loader2, Mail, Lock, User, BookOpen,
  Image as ImageIcon, Stethoscope, MessageSquare,
} from "lucide-react";
  ...
  <div className="hidden lg:flex lg:w-1/2 bg-slate-900 relative overflow-hidden p-12 flex-col justify-between">
    ...
    <div className="relative z-10">
      <h2 className="text-3xl font-black text-white leading-tight mb-4">
        Smarter livestock health starts here.
      </h2>
      <p className="text-slate-400 text-sm leading-relaxed mb-9 max-w-sm">
        Create your account to detect cattle diseases from images or symptoms, get instant
        treatment guidance, and track every diagnosis in one place.
      </p>
...
const SignUp = () => {
  ...
  const mutation = useMutation({
    ...
    onSuccess: () => {
      navigate("/signin", { state: { message: "Account created successfully! Please login." } });
    },
    onError: (error) => {
      ...
      <div className="flex-1 flex items-center justify-center p-6 sm:p-10">
        <div className="w-full max-w-md">
          ...
          <div className="mb-9">
            ...
            <p className="text-slate-500 text-sm">Join VetAI Diagnostics — it only takes a minute.</p>
          </div>
            ...
            <div className="space-y-1">
              ...
              <div className="relative group">
                ...
                <input
                  ...
                  className={`w-full pl-11 pr-4 py-3.5 text-sm rounded-xl border bg-white transition-all outline-none ${errors.email
                      ? "border-red-200 focus:ring-2 focus:ring-red-100 focus:border-red-400"
                      : "border-slate-200 focus:ring-4 focus:ring-blue-50/50 focus:border-blue-500"
                    } ${isLoading ? "bg-slate-50 cursor-not-allowed opacity-70" : ""}`}
                />
              ...
              {errors.email && (
                <p className="text-red-500 text-[11px] font-medium mt-1.5 ml-1 animate-in fade-in duration-200">{errors.email.message}</p>
              )}
            </div>
